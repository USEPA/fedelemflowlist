...
`fedelemflowlist` is a Python package that generates and provides a standardized elementary flow list for use in life cycle assessment (LCA) data
 ...
 where preferred flows from the active version of the flow list produced by this package can be found. 
 
 Standard formats for a [Flow List](./format%20specs/FlowList.md)
 and a [Flow Mapping](./format%20specs/FlowMapping.md) are defined and used in `fedelemflowlist`. They are implemented as [pandas](https://pandas.pydata.org/) dataframes. 
...
contents, use examples, and for how to contribute to the flow list through additions or edits to flows.
