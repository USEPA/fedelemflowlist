...
def count_flows_by_class(flowlist):
    ...
    fl_combined['Percent of Flows'] = fl_combined['No. of Flows'].apply(lambda x: str((x / total_flows)*100) + '%')
    total_row = {'Class':'TOTAL','No. of Flows':total_flows,'No. of Preferred Flows':fl_combined['No. of Preferred Flows'].sum()}
...
def list_contexts(flowlist):
    ...
    contexts['Num'] = 1
    context_counts = contexts.pivot_table(index='Context',columns='Class',values='Num',aggfunc='count',fill_value='0').reset_index()
...
def count_flowables_by_class(flowlist):
    ...
    flowables_by_class_count['Percent'] = flowables_by_class_count['Flowable'].apply(lambda x: str((x / total)*100) + '%')
